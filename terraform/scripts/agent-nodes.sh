...
# Wait until server is reachable before starting agent
echo "Waiting for RKE2 server at https://$DOMAIN:9345 ..."
until curl -sk --connect-timeout 5 https://$DOMAIN:9345/ping; do
  echo "Server not ready yet, retrying in 5s..."
  sleep 5
done
echo "RKE2 server is reachable. Starting agent..."
